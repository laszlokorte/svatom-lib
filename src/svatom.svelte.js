...
export function combine(mapOfAtoms, writables = null) {
  if (writables === null) {
    writables = R.mapObjIndexed((v, k) => {
      return (
        typeof Object.getOwnPropertyDescriptor(v, "value").set === "function"
      );
    }, mapOfAtoms);
  }
...
export function combineWithRest(mapOfAtoms, rest = atom({}), writables = null) {
  ...
  return {
    ...
    set value(newVal) {
      ...
      rest.value = R.pick(
        R.difference(Object.keys(newVal), Object.keys(mapOfAtoms)),
        newVal,
      );
    },
  };
...
export function bindScroll(node, someAtom) {
  ...
  const onScrollThrottled = function onscroll(e) {
    ...
    const nodeScrollLeft = Math.round(node.scrollLeft);
    const nodeScrollTop = Math.round(node.scrollTop);

    if (newValueX != nodeScrollLeft || newValueY != nodeScrollTop) {
